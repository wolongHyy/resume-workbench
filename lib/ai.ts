...
const COACH_OUTPUT = {
  structure: { profile: {}, education: [], experience: [], internships: [], projects: [], campus: [], awards: [], skills: [] },
...
function fallback(task: AiTask, resume: Resume, input: string) {
  ...
  if (task === "coach") {
    return {
      ...
      jd: { raw: input, keywords: [], requirements: [] },
      matching: { matched: [], missing: [], keywords: [], score: 0 },
      risks: [{ type: "事实核查", path: "全局", item: "请确认简历中的经历、数字、学校与公司均来自真实事实", reason: "无法在缺少原始事实来源的情况下核验", severity: "info" }],
      suggestions: [{ path: "summary", before: "", after: "待补充", reason: "缺少个人概述，请补充真实信息" }],
      missing: ["个人概述"],
      warnings: ["本机 AI 工具当前不可用，已生成保守建议；请逐项补充真实信息后再使用。"],
      needsConfirmation: true,
    };
  }
  return { suggestions: [{ path: "summary", before: resume.summary, after: resume.summary || "具备从需求调研、方案设计到独立开发落地的完整实践经验。" }], note: "本机 AI coding CLI 当前不可用，已生成可编辑的保守建议。" };
}
...
export async function runSkill(task: AiTask, resume: Resume, input = "") {
  ...
  const taskOutput = task === "optimize"
    ? '{"suggestions":[{"path":"summary","before":"","after":"","reason":""}],"warnings":[],"needsConfirmation":true}'
    : task === "match"
      ? '{"keywords":[],"matched":[],"missing":[],"score":0,"warnings":[],"needsConfirmation":true}'
      : task === "check"
        ? '{"score":0,"warnings":[],"passed":false,"needsConfirmation":false}'
        : task === "coach"
          ? JSON.stringify(COACH_OUTPUT)
          : '{"profile":{},"education":[],"projects":[],"experience":[],"skills":[],"warnings":[],"needsConfirmation":true}';
  ...
  return new Promise<unknown>((resolve) => {
    const child = spawn(process.env.CODEX_BIN || "codex", ["exec", "--ephemeral", "--skip-git-repo-check", "--output-last-message", outputFile, prompt], { cwd: skillsDir, windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
    let stderr = "";
